...
// 
template <typename T, size_t R>
void initialize(tensor<T, R>&, const T, const T);
...
public:
    ...
    // ------------------------------------------------------------------------------------------------------
    /// @brief  Initializes the tensor with values between a specified range
    /// @param[in]  tensr   The tensor to initialize
    /// @param[in]  min     The minimum value to initialize with
    /// @param[in]  max     The maximum value to initialize with
    // ------------------------------------------------------------------------------------------------------
    friend void initialize<T, R>(tensor& tensr, const T min, const T max);
...
template <typename T, size_t R>
void initialize(tensor<T, R>& tensr, const T min, const T max)
{
    std::random_device                  rand_device;
    std::mt19937                        gen(rand_device());
    std::uniform_real_distribution<>    dist(min, max);
    for (auto& element : tensr._data) {
        element = static_cast<T>(dist(gen));        // For ints
    }
}    
