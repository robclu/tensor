...
#include "tensor_expressions_static.hpp"
...
// ----------------------------------------------------------------------------------------------------------
template <typename T, tensor_type Type, size_t SF, size_t... SR>
...
// ----------------------------------------------------------------------------------------------------------
template <typename T, size_t SF, size_t... SR>
class static_tensor : tensor_expression<static_tensor<T, SF, SR...>> {
public:
    // --------------------------------------- TYPEDEFS -----------------------------------------------------
    using data_type         = T;
    ...
    static constexpr tensor_type type = tensor_type::STATIC;
    // ------------------------------------------------------------------------------------------------------
    ...
    static_tensor(const tensor_expression<Expression>& expression);
...
static_tensor<T, SF, SR...>::static_tensor(const tensor_expression<E>& expression) 
{   
    // Convert the nano::list of dimension sizes to a constant array
    _dim_sizes = nano::runtime_converter<typename container_type::dimension_sizes>::to_array();  
